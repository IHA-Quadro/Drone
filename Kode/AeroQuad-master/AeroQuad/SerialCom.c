...
void InvertUART()
{
	PrintHelper.PrintConfig[STATUSMODE] = !PrintHelper.PrintConfig[STATUSMODE];
	PrintHelper.PrintConfig[DEBUGMODE] = !PrintHelper.PrintConfig[DEBUGMODE];
}
...
void StopMotors()
{
	...
	{
		PrintHelper.printStatus("Motor stopped!");
		InvertUART();
	}
	else
	{
		ResetInputData();
		InvertUART();
		PrintHelper.printStatus("Motor started!");
	}
}
